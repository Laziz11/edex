...
process.on("uncaughtException", e => {
    signale.fatal(e);
    dialog.showErrorBox("Luke's Terminal crashed", e.message || "Cannot retrieve error message.");
    if (tty) {
        tty.close();
    }
    if (extraTtys) {
...
signale.start(`Starting Luke's Terminal v${app.getVersion()}`);
